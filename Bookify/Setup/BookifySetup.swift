...
enum SetupError: Error, LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case let .hotelsRegistrationFailed(error): return "Hotels registration failed: \(error)"
        case let .bookingsRegistrationFailed(error): return "Bookings registration failed: \(error)"
        }
    }
}
...
enum BookifySetup {
    ...
    static func bootstrap(bundle: Bundle = .main) throws {
        let prerequisites = try makePrerequisites(from: bundle)
        try applyPrerequisites(prerequisites)
    }
    ...
    private static func applyPrerequisites(_ prerequisites: Prerequisites) throws {
        // ✅ 1) Map buildInfo first
        let enrichedConfig = prerequisites.config.with(buildInfo: buildInfo())
