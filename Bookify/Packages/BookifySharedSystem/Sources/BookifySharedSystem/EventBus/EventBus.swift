...
public final class EventBus: EventBusType {
    ...
    private func subject<E: AppEvent>(for type: E.Type) -> PassthroughSubject<E, Never> {
        ...
        lock.lock()
        defer { lock.unlock() }
        if let subject = eventSubjects[key] as? PassthroughSubject<E, Never> { return subject }
        let subject = PassthroughSubject<E, Never>()
        eventSubjects[key] = subject
        return subject
    }
    ...
    public func publisher<E: AppEvent>(_: E.Type) -> AnyPublisher<E, Never> {
